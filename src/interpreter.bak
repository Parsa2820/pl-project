...
(module interpreter racket
  ...
  (define value-of-program
    (lambda (prgm env)
      (cases program prgm
        (program-base (sts) (value-of-stmts sts env))))
    )
  ...
  (define value-of-simple-st
    (lambda (st env)
      (cases simple-st st
        (assignment-st (lhs rhs) (if (null? env) (extend-env lhs (newref (value-of-exp rhs)) global-env) (extend-env lhs (newref (value-of-exp rhs)))))
  ...
  (define call-no-input
    (lambda (function env)
      (cases function-datatype function
        (function-no-input (fun-name  fun-stmts)  (value-of stmts))))) 
